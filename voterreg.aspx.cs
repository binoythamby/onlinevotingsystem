...
public partial class voterreg : System.Web.UI.Page
{
    ...
    {
        if (DB.ExecuteReader("select userid from votertab where userid='" + TextBox1.Text + "'").Read())
        {
            Response.Redirect("voterreg.aspx?regerror=true");
        ...
        else
        {
            ...
            DB.ExecuteNonQuery("insert into votertab (fname,lname,fnm,dob,address,email,gender,city,userid,pass,image,contact,status ) values(@fname,@lname,@fnm,@dob,@address,@email,@gender,@city,@userid,@pass,@image,@contact,'Pending')", p);
           
            FileUpload1.SaveAs(Server.MapPath("vimage/") + FileUpload1.FileName);
